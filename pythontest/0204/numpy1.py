...
print(arr)
arr = np.full((3,5), 3.14) # 3 행 5열 리스트 생성. 3.14로 채움
print(arr)

# 0에서 시작해 2씩 더해 20 까지 채우는 배열 생성
arr = np.arange(0, 20, 2)
print(arr)
# 0 과 1 사이에 일정한 간격을 가진 다섯개의 값으로 채운 배열 만들기
arr = np.linspace(0, 1, 5)
print(arr)
# 3 * 3 크기의 난수 배열 생성
arr = np.random.random((3, 3))
print(arr)
# 평균 0, 표준 편차 1 의 정규 분포를 가지는 3 * 3 난수 배열
...
print(arr.ndim) # 배열에 차원 리턴  -> 2
print(arr.shape) # 각 차원의 크기 리턴 - ( 3, 3)
